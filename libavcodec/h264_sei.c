...
#define LITTLE_ENDIAN_MAPTYPE_BYTE 16
#define LITTLE_ENDIAN_MAPTYPE_BYTE 19
#define MIN_PINCH_TO_ZOOM_DATA_SIZE 36
...
static inline uint32_t endianSwappedInt32(uint8_t* in) {
...
static int decode_unregistered_user_data(H264Context *h, int size)
{
    ...
    int maxBytesSaved = sizeof(uint32_t)*PTZ_DATA_NUM_ELEMENTS;
    int bytesToSave = size < maxBytesSaved ? size : maxBytesSaved;
    int e, build, i, j;
    ...
    if (size >= MIN_PINCH_TO_ZOOM_DATA_SIZE) {
        uint32_t map_type_index = PTZ_MAP_TYPE_INDEX*sizeof(uint32_t);
        uint8_t* map_type_ptr = &user_data[map_type_index];
        uint32_t map_type_le = endianSwappedInt32(map_type_ptr);
        uint32_t map_type_be = *(uint32_t*)map_type_ptr;

        av_log(h->avctx, AV_LOG_DEBUG, "decode_unregistered_user_data map_type_index: %d", map_type_index);
        av_log(h->avctx, AV_LOG_DEBUG, "decode_unregistered_user_data test (le, be): %d, %d", map_type_le, map_type_be);
        if (map_type_le == MAPTYPE_EPTZ) {
            av_log(h->avctx, AV_LOG_DEBUG, "decode_unregistered_user_data PTZ found (little-endian) size: %d", size);
            for (j = 0; j < PTZ_DATA_NUM_ELEMENTS && j*sizeof(uint32_t) < size; ++j) {
                uint8_t* data_ptr = &user_data[j*sizeof(uint32_t)];
                h->sei_unregistered_user_data[j] = endianSwappedInt32(data_ptr);
            }
        } else if (map_type_be == MAPTYPE_EPTZ) {
            av_log(h->avctx, AV_LOG_DEBUG, "decode_unregistered_user_data PTZ found (big-endian) size: %d", size);
            memcpy(h->sei_unregistered_user_data, user_data, bytesToSave);
        }
    }
...
int ff_h264_decode_sei(H264Context *h)
{
    while (get_bits_left(&h->gb) > 16 && show_bits(&h->gb, 16)) {
        ...
        if (size > get_bits_left(&h->gb) / 8) {
            ...
            av_log(h->avctx, AV_LOG_ERROR, "SEI buffer %p buffer end %p",
                   h->gb.buffer, h->gb.buffer_end);
